    ...
    public Optional<StorageContentObject> getStorageObjectByPath(String path) {
        return lookupStorage(path, subjectKey, authToken)
                ...
                .flatMap(s -> {
                    ...
                    // TODO:
                    //  the storage lookup API unfortunately doesn't provide information about the requested object,
                    //  (for example, isCollection is hardcoded to true)
                    //  so we need to do another query here. Ideally, this should be fixed in the Jade API.
                    ...
                    return listContent(storageInfo.getStorageURL(), location.getRelativePath().toString(), 1, subjectKey, authToken)
                            ...
                            // Another case is when we ask for a non-collection, the entryRelativePath gets filled in
                                    || c.getMainRep().getRemoteInfo().getEntryRelativePath().equals(storageInfo.getEntryRelativePath()))
